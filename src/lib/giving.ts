export type GivingCategory = "tithe" | "offering";
export type GivingAccount = { id: string; imageUrl: string; bankName: string; accountName: string; accountNumber: string; currency: string; note: string; category: GivingCategory; visible?: boolean };
...
export function normalizeGivingAccounts(data: Record<string, unknown> | undefined): GivingAccount[] {
  ...
  if (typeof data?.accountNumber === "string" && data.accountNumber.trim()) {
    return [{ id: "legacy-giving-account", imageUrl: typeof data.imageUrl === "string" ? data.imageUrl : "", bankName: typeof data.bankName === "string" ? data.bankName : "", accountName: typeof data.accountName === "string" ? data.accountName : "", accountNumber: data.accountNumber, currency: typeof data.currency === "string" ? data.currency : "NGN", note: typeof data.note === "string" ? data.note : "", category: data.category === "tithe" ? "tithe" : "offering", visible: data.visible !== false }];
  }
  return [];
}
